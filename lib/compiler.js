...
function Compiler(forth) {
    this.compile = function (text, indef) {
        ...
        function processNative(value) {
            const arity = forth.getNativeArity(value);
            ...
            if (queue.length >= arity) {
                if (arity === 2) {
                    ...
                    queue.push("(" + op1 + " " + value + " " + op2 + ")");
                }
                else if (arity === 1) {
                    const op = queue.pop();
                    const oper = forth.getNativeOperation(value);
                ...
                appendToResult("forth.apply('" + value + "');");
            }
        }
